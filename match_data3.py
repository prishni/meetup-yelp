...
def search_reviews_from_dataset(yelp_id, yelp_name, yelp_complete_review_dataset):
    ...
    for review_lines in yelp_review_dataset_fh.readlines():
        if yelp_id in review_lines:
            for m in re.finditer('"text": "', review_lines):
                substr = review_lines[int(m.end()):len(review_lines)]
                end_loc = substr.find('"')
                substr = substr[:end_loc]

                if substr not in reviews:
                    reviews.append(substr)
            ...
            if data[a]["city"] == city:
                ...
                if meetup_id == meetup_id_for_search:
                    ...
                    if yelp_name == 0 and yelp_id == 0 and len(phone) > 4 and Match == False:
                        ...
                        yelp_id, yelp_name, yelp_categories = extract_phone_number(yelp_completed_business_dataset,
                                                                                   phone)
                        if yelp_name != 0 and yelp_id != 0:
                            print(yelp_name)
                            write_yelp_meet_tofile(meetup_id, yelp_id, yelp_name)
