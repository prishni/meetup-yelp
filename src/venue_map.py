...
def find_by_phone_api(phone, yelp_client):
    ...
    try:
        ...
        if not 'error' in json_output:
            ...
            # fh=open(yelp_completed_dataset_file_name,'r')

            # for line in fh.readlines():
            #     if '"phone"' in line:
            #         loc=int(line.find('"phone"'))
            #         loc=int(loc+10)
            #         temp_line=line[loc:]
            #         loc_end=int(temp_line.find('",'))
            #         temp_line=temp_line[:loc_end]
            #
            #         print(temp_line)

            # if line.get('phone'):
            #     yelp_phone=line.get('phone')
            #     yelp_phone=re.sub('[^0-9]+', '', yelp_phone)
            #     if phone in yelp_phone:
            #         print(str(phone)+' '+str(yelp_phone)+' '+line.get('name'))
        ...
        if not 'error' in json_output:
            ...
            if int(total) > 0:
                ...
                if len(nearest_loc_id) > 0 and len(nearest_loc_name) > 0:
                    return nearest_loc_name, nearest_loc_id
                else:
                    return 0, 0
            else:
                return 0, 0
            ...
            if data[a]["city"] == city:
                ...
                if meetup_id == meetup_id_for_search:
                    ...
                    if len(phone) > 4 and yelp_name == 0 and yelp_id == 0 and Match == False:
                        ...
                        yelp_name, yelp_id = find_by_phone_api(phone, yelp_client)
                        print ('ID'+yelp_id)
                        if yelp_name != 0 and yelp_id != 0:
                            ...
                            Match = True
                            return yelp_id, yelp_name, reviews
                    if len(name) > 0 and yelp_name == 0 and yelp_id == 0 and Match == False:
                        print('Name Match from API')
                        yelp_name, yelp_id = find_by_name_api(name, yelp_client, lat, lon)
                    ...
                    if len(name) > 0 and yelp_name == 0 and yelp_id == 0 and Match == False:
                        ...
                        if check_file_exist(yelp_file) == True:
                            yelp_name, yelp_id = find_by_name_pattern(name, lat, lon, yelp_file)
                    ...
                    if yelp_name == 0 and yelp_id == 0 and len(phone) > 4 and Match == False:
                        ...
                        yelp_id, yelp_name = extract_phone_number(yelp_completed_business_dataset, phone)
                        if yelp_name != 0 and yelp_id != 0:
                            print(yelp_name)
                            write_yelp_meet_tofile(meetup_id, yelp_id, yelp_name)
                            reviews = search_reviews_from_dataset(yelp_id, yelp_name, yelp_complete_review_dataset)
...
def main1(arg_list):
    ...
    populate_api_list()
    yelp_id, yelp_name, reviews = map_meetup_yelp_data(city, meetup_id_for_search, meetup_file_name,
                                                       yelp_completed_business_dataset,
                                                       yelp_complete_review_dataset)
    f=open('try_write.txt','w')
    print(yelp_id + ' ' + yelp_name + ' ' + str(reviews))
    f.write(yelp_id + ' ' + yelp_name + ' ' + str(reviews))
    f.close()
    return yelp_id,str(reviews)
#main1(arg_list)
